...
export const testMessages = {
  fraudulent: [
    {
      sender: '+1234567890',
      message: 'Congratulations! You have won $10,000 in our lottery! Click here to claim your prize: http://suspicious-link.com/claim',
    },
    {
      sender: 'BANK-ALERT',
      message: 'URGENT: Your account has been suspended due to suspicious activity. Verify your identity immediately by clicking: www.fake-bank.com/verify',
    },
    {
      sender: '+9876543210',
      message: 'You have been selected for a special offer! Get free money now! Limited time only. Act fast: bit.ly/freemoney123',
    },
    {
      sender: 'TAX-REFUND',
      message: 'You are eligible for a tax refund of $2,500. Confirm your social security number and bank details: www.fake-irs.com',
    },
  ],
  safe: [
    {
    ...
    {
      sender: 'DELIVERY',
      message: 'Your package will be delivered today between 2-4 PM. Track your order at www.legitimate-courier.com',
    },
    {
      sender: '+2222222222',
      message: 'Thanks for the documents. I will review them and get back to you by tomorrow.',
    },
    {
      sender: 'BANK',
      message: 'Your monthly statement is now available. Login to your account using the official mobile app to view details.',
    },
  ],
};
 ...
 */
export const simulateIncomingSMS = async (message: { sender: string; message: string }) => {
  try {
    ...
    const result = await smsMonitorService.scanMessage(message.message, message.sender);
...
export const runFullSMSTest = async () => {
  ...
  for (const testMsg of testMessages.fraudulent) {
    try {
      ...
      console.log(`✅ Fraudulent test passed: ${testMsg.message.substring(0, 50)}...`);
    } catch (error) {
      console.error(`❌ Fraudulent test failed: ${testMsg.message.substring(0, 50)}...`, error);
  ...
  for (const testMsg of testMessages.safe) {
    try {
      ...
      console.log(`✅ Safe test passed: ${testMsg.message.substring(0, 50)}...`);
    } catch (error) {
      console.error(`❌ Safe test failed: ${testMsg.message.substring(0, 50)}...`, error);
      results.errors.push({ ...testMsg, error });
    }
  }
