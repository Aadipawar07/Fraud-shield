...
// NOTE: Using dangerouslyAllowBrowser is not recommended for production applications
// as it may expose your API key to the client. In a real app, you should use a backend API.
const client = new OpenAI({
  // Important: Replace with your own API key in .env file
  // For this demo, we use a placeholder key that will cause the fallback to be used
  apiKey: process.env.OPENAI_API_KEY || 'sk-placeholder-key-replace-with-real-one', 
  dangerouslyAllowBrowser: true // Enable browser usage (for demo purposes only)
});
...
export async function analyzeMessage(
  ...
  // Primary approach: Use OpenAI fraud detection
  if (useAI) {
    try {
      ...
      if (fraudResult.error) {
        console.error('Fraud detection error:', fraudResult.error);
        ...
        if (fraudResult.classification === 'FRAUD') {
          ...
          result.matchedPatterns.push('AI detection');
          result.reason = fraudResult.reason || 'Advanced AI analysis identified this as potentially fraudulent';
          return result;
        } 
          ...
          result.score = scoreValue;
          result.confidence = confidence;
          result.reason = fraudResult.reason || 'AI verified this as a legitimate message';
          return result;
        }
          ...
          result.score = scoreValue;
          result.confidence = confidence;
          result.reason = fraudResult.reason || 'AI verified this as a normal message';
          return result;
        }
      }
    } catch (error) {
...
/**
 * Calls the OpenAI API to classify an SMS message as FRAUD, LEGITIMATE, or NORMAL_SMS
...
export async function detectFraud(message: string): Promise<FraudDetectionResponse> {
  ...
  const fallbackResponse = (errorMsg: string): FraudDetectionResponse => {
    console.log("Using local detection as fallback due to:", errorMsg);
    ...
    return {
      ...
      reason: `${localResult.reason} (Local fallback - API error: ${errorMsg})`,
      error: errorMsg
    };
  };
  ...
  try {
    console.log("Starting OpenAI API call for fraud detection...");
    
    const completion = await client.chat.completions.create({
      model: "gpt-3.5-turbo", // Using 3.5-turbo as GPT-5 isn't available yet
      messages: [
        { role: "system", content: fraudDetectionPrompt },
        { role: "user", content: message },
      ],
      temperature: 0, // keep it deterministic for consistent output
    ...
    try {
      return JSON.parse(result) as FraudDetectionResponse;
    } catch (parseError) {
    ...
    // If the error is related to browser environment
    if (errorMessage.includes("browser") || errorMessage.includes("dangerously")) {
      console.error("Browser compatibility issue:", error);
      errorType = "Browser compatibility issue with OpenAI SDK";
    }
    // If it's an API key issue
    else if (errorMessage.includes("API key") || errorMessage.includes("authentication") || 
             errorMessage.includes("auth") || errorMessage.includes("key")) {
      console.error("API authentication issue:", error);
      errorType = "OpenAI API authentication failed - check your API key";
    }
    // If it's a network error
    else if (errorMessage.includes("network") || errorMessage.includes("connect") || 
             errorMessage.includes("CORS") || errorMessage.includes("timeout")) {
      console.error("Network issue:", error);
      errorType = "Network issue connecting to OpenAI API";
    }
    // Rate limit errors
    else if (errorMessage.includes("rate") || errorMessage.includes("limit") || 
             errorMessage.includes("429")) {
      console.error("Rate limit issue:", error);
      errorType = "OpenAI API rate limit exceeded";
    }
    // Quota errors
    else if (errorMessage.includes("quota") || errorMessage.includes("billing") || 
             errorMessage.includes("exceed")) {
      console.error("Quota issue:", error);
      errorType = "OpenAI API quota exceeded - check your billing";
    }
    // Model issues
    else if (errorMessage.includes("model") || errorMessage.includes("not found") || 
             errorMessage.includes("unavailable")) {
      console.error("Model issue:", error);
      errorType = "OpenAI model unavailable or incorrect";
    }
    // Generic error fallback
    else {
      console.error("Unspecified OpenAI error:", error);
      errorType = `OpenAI API error: ${errorMessage.substring(0, 100)}`;
    }
 ...
 * Please use detectFraud() instead.
 */
