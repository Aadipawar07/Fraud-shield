...
function simpleSpamDetection(message) {
    ...
    const fraudKeywords = [
        // Prize/lottery scams
        'won', 'winner', 'congratulations', 'claim', 'prize', 'lottery', 'jackpot', 'reward',
        // Urgency tactics
        'urgent', 'immediately', 'expires', 'limited time', 'act now', 'deadline', 'last chance', 'hurry', 'asap', 'quick',
        // Verification/security scams
        'verify', 'confirm', 'update', 'suspended', 'blocked', 'security alert', 'account locked', 'kyc', 'otp',
        // Financial scams
        'free money', 'cash', 'refund', 'tax refund', 'stimulus', 'inheritance', 'investment', 'loan approved', 'credit approved',
        // Banking and id terms (regional)
        'bank', 'upi', 'ifsc', 'aadhaar', 'pan', 'gst',
        // Suspicious links/actions
        'click here', 'click link', 'download', 'install app', 'visit website', 'open link', 'http', 'https', 'www.', 'bit.ly', 'tinyurl',
    ];
    ...
    const hasUrl = /(https?:\/\/|www\.|bit\.ly|tinyurl|t\.co|shorturl)/i.test(message);
    const hasMoney = /(?:\$|₹|rs\.?|inr\s?)?[0-9][0-9,]*(?:\.[0-9]{1,2})?/.test(message) || /(prize|cash|money)/i.test(message);
    const hasUrgency = /(urgent|immediately|act now|expires|last chance)/i.test(message);
    ...
    const hasWinWords = /(congratulations|you\s*(have)?\s*won|winner|prize|lottery)/i.test(text);
    ...
    if (hasMoney && hasWinWords) suspicionScore += 0.30; // amount + winnings wording
    ...
    try {
        ...
        try {
            ...
            const predictions = Array.isArray(modelResponse)
                ? (Array.isArray(modelResponse[0]) ? modelResponse[0] : modelResponse)
                : [];

            // Determine spam probability robustly
            const findByLabel = (labels) => predictions.find(p => labels.includes(String(p.label).toUpperCase()));
            const spamEntry = findByLabel(["SPAM", "LABEL_1"]) || findByLabel(["LABEL1", "TOXIC"]); // fallbacks
            const hamEntry = findByLabel(["HAM", "LABEL_0"]) || findByLabel(["LABEL0"]);

            let spamProb = 0;
            if (spamEntry && typeof spamEntry.score === 'number') spamProb = spamEntry.score;
            else if (hamEntry && typeof hamEntry.score === 'number') spamProb = 1 - hamEntry.score;
            else if (predictions.length > 0) {
                // fallback: take highest score and map by label name
                const top = predictions.reduce((a, b) => (a.score > b.score ? a : b));
                spamProb = String(top.label).toUpperCase().includes('SPAM') ? top.score : 1 - top.score;
            }
...
app.post("/verify-number", (req, res) => {
    try {
        ...
        try {
            ...
            blacklisted = Array.isArray(json.blacklistedNumbers) ? json.blacklistedNumbers : [];
        } catch (e) {
        ...
        const phoneEntries = blacklisted.filter((e) => e && e.type === "phone" && e.id);
        const matches = phoneEntries.filter((entry) => {
            const entryDigits = normalizePhoneNumber(entry.id);
            return normalized === entryDigits || normalized.endsWith(entryDigits) || entryDigits.endsWith(normalized);
        });
